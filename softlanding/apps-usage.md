...
### Rectangle (무료) — 창 분할
- **왜?** 윈도우 Win+화살표 대체. **bootstrap 이 기본 설치** (무료, 오픈소스)
- 권한: 손쉬운 사용 ON
- 단축키: `⌃⌥←` 왼쪽 절반, `⌃⌥→` 오른쪽 절반, `⌃⌥Enter` 전체

> 💸 **무료 vs 유료** — 창 관리는 둘 중 하나만 쓰면 됩니다.
> - **Rectangle (무료, 기본)** — 단축키 기반 창 분할. 대부분 충분.
> - **Magnet (유료, App Store ~$9.99)** — 화면 가장자리로 드래그하는 스냅 UI 가 더 직관적이라 선호하는 사람도 많음. 유료라 자동 설치 안 함 — 원하면 App Store 에서 직접 구매.
> 둘은 단축키가 겹칠 수 있으니 **하나만** 쓰세요.
...
### RunCat — 메뉴바 CPU 인디케이터
- 메뉴바에서 고양이가 CPU 부하에 따라 빠르게/느리게 달림 (한눈에 부하 체감)
- Stats(상세 수치)와 역할이 다름 — 같이 써도 됨
- App Store 설치. 첫 실행 후 메뉴바 아이콘 → Preferences 에서 캐릭터 변경 가능
...
### Discord
- 팀 커뮤니티/음성 채널용. 첫 실행 → 로그인 → 팀 서버 참여
- 음성 채널 처음 들어가면 **마이크 권한** 요청 → 시스템 설정에서 허용
- 알림 과하면: 서버 우클릭 → 알림 설정 → @멘션만
